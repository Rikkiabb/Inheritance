...
void World::resetOrganisms() {
    for (int i = 0; i < WORLDSIZE; i++){
        ...
        for (int j = 0; j < WORLDSIZE; j++){
            ...
            if (grid[i][j] != NULL){
                // Ny umferd hefst og vid stillum allt hasMoved a false
...
void World::moveOrganism(OrganismType aType) {
    for (int i = 0; i < WORLDSIZE; i++){
        ...
        for (int j = 0; j < WORLDSIZE; j++){
           // Ef grid er NULL, ta faerum vid tad ekki
            if (grid[i][j] != NULL){
                
                // Vinnum hvort ad grid er af somu gerd og aType og hvort tad hefur hreyft sig
                // Ef tau skilyrdi eru uppfyllt ta faerum vid tad
...
void World::breedOrganisms() {
    for (int i = 0; i < WORLDSIZE; i++){
        ...
        for (int j = 0; j < WORLDSIZE; j++){
            ...
            // Ef tad er NULL ta getur tad ekki breedad
            if (grid[i][j] != NULL){
                
                // Tarf ad faera sig til ad breeda
                if (grid[i][j]->hasMoved()){
                    
                    // Skodum hvort tad eigi ad breeda
                    grid[i][j]->breed();
                }
            }
        }
